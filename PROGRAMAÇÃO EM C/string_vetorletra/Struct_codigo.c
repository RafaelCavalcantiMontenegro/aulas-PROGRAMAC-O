...
int buscarPeloIdade(struct Pessoa v[], int tam, int x);
...
*/
  printf("Digite uma idade a ser encontrado: ");
  scanf("%d", &idade);

  pos_2 = buscarPeloIdade(pessoas, QTD_PESSOAS, idade);
  if (pos != -1) {
    printf("Pessoas encontrada: ");
    imprimirPessoa(pessoas[pos_2]);
  } else {
...
void insercaoDiretaCampoIdade(struct Pessoa v[], int n) {
  ...
  for (i = 1; i <= n - 1; i++) {
    chave = v[i];
    j = i - 1;
    while (j >= 0 && v[i].idade < v[i+1].idade) {
      v[j+1] = v[j];
      j = j - 1;
    }
    v[j+1] = chave;
